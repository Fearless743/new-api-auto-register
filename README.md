...
一个用于批量注册、定时签到、定时刷新余额，并通过本地 Web API 提供余额查询的工具。
...
- 批量注册账号并保存到本地数据文件
- 定时执行签到任务
- 每 10 分钟刷新一次真实余额并写入本地缓存
- 提供本地 Web API 查询缓存余额，不直接请求上游接口
...
- 仓库是公开仓库，不要把真实敏感凭据直接提交到 git
- `store.json` 中可能包含账号信息、token 和 session，部署时应仅保存在服务器本地
- API 返回的是本地缓存余额，不是实时请求结果
